...
function createGroceries(): string[] {
  ...
  let groceries: string[] = ["Eggs", "Milk", "Rise", "Chicken", "Oil", "Meat"];
...
function getLastGroceryItem(groceries: string[]): string {
  ...
  let lasIndex = groceriesLength - 1;

  return groceries[lasIndex]; // replace empty string with what you see is fit
}
...
function removeLastGroceryItem(groceries: string[]): string {
  ...
  return last; // replace empty string with what you see is fit
}
...
function getFirstThreeGroceryItems(groceries: string[]): string[] {
  // write your code here...
  let newArray: string[] = groceries.slice(0, 3);
